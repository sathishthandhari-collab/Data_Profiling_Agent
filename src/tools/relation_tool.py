            ...
            for s_col in source_cols:
                ...
                for t_col in tgt_candidates:
                    ...
                    if s_col not in src_distinct:
                        src_distinct[s_col] = (
                            source_df.select(F.col(s_col).alias("k")).where(F.col(s_col).isNotNull()).distinct().cache()
                        )
                        # materialize so we don't recompute for each target comparison
                        src_distinct[s_col].count()
                    if t_col not in tgt_distinct:
                        tgt_distinct[t_col] = (
                            target_df.select(F.col(t_col).alias("k")).where(F.col(t_col).isNotNull()).distinct().cache()
                        )
                    ...
                    inter = src_vals.join(tgt_vals, on="k", how="leftsemi").agg(F.approx_count_distinct("k").alias("n")).collect()[0]["n"]
